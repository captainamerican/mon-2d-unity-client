...
namespace Battle {
	...
	public class Combatant {
		...
		public bool HasStatus(Game.Status status) {
			return Statuses.Any(cs => cs.Status == status);
		}

		public void AddStatus(Game.Status status, int turns, float strength) {
		...
		public void Update() {
			...
			Statuses.ForEach(cs => {
				switch (cs.Status) {
					case Game.Status.StrengthBuff:
					case Game.Status.StrengthDebuff:
						strengthAdjustment += cs.Strength;
						break;

					case Game.Status.EnduranceBuff:
					case Game.Status.EnduranceDebuff:
						enduranceAdjustment += cs.Strength;
						break;

					case Game.Status.DexterityBuff:
					case Game.Status.DexterityDebuff:
						dexterityAdjustment += cs.Strength;
						break;

					case Game.Status.IntelligenceBuff:
					case Game.Status.IntelligenceDebuff:
						intelligenceAdjustment += cs.Strength;
						break;

					case Game.Status.WisdomBuff:
					case Game.Status.WisdomDebuff:
						wisdomAdjustment += cs.Strength;
						break;

					case Game.Status.LuckBuff:
					case Game.Status.LuckDebuff:
		...
		public int Experience() {
			return Mathf.RoundToInt((Strength + Endurance + Dexterity + Intelligence + Wisdom + Luck) * (1 + (0.2f * Level)));
		}
	}
